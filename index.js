...
const createWSConnection = require('./ws.create')
...
// WS ===========================================================================
const url = 'wss://y5n27bi22i.execute-api.ap-southeast-1.amazonaws.com/staging'
const wsOptions = {
  headers: {
    'au-payload': JSON.stringify({
      userId: 182312
    })
  }
}
...
async function test(n_connection = 10000, steps = 100) {
  ...
  while (i < n_connection) {
    ...
    while (i1 < n) {
      ...
      const con = createWSConnection(url, i1 + i, wsOptions)
      i1 ++
    }
    i += n
  }
...
const n_connection = connectArgv || 10 // 15000
test(n_connection, 10)
